...
-(void)selfSelectionImageName:(NSString *)imgName labelString:(NSString *)string balanceText:(NSString *)balanceString{
    ...
    _labelText.textColor = H_SUBSTRACT;
    ...
    if (labelSize.width > selImg.size.width || labelSize.width == selImg.size.width){
        ...
        _labelText.frame = FRAMEMAKE_F((labelSize.width - selImg.size.width)/2 - 6, _selfImg.image.size.height + 10, labelSize.width, labelSize.height);
    }
    
    if (labelSize.width < selImg.size.width){
        _labelText.frame = FRAMEMAKE_F((selImg.size.width - labelSize.width) / 2,_selfImg.image.size.height, labelSize.width, labelSize.height);
