...
- (JFSearchView *)searchView {
    if (!_searchView) {
        ...
        _searchView = [[JFSearchView alloc] initWithFrame:CGRectMake(0, 44, frame.size.width, frame.size.height  - 104)];
...
- (NSMutableArray *)characterMutableArray {
    if (!_characterMutableArray) {
        _characterMutableArray = [NSMutableArray arrayWithObjects:@"定位", @"最近", @"热门", nil];
    }
...
- (UITableView *)rootTableView {
    if (!_rootTableView) {
        _rootTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, WIDTH_K, HEIGHT_K - 64) style:UITableViewStylePlain];
        _rootTableView.delegate = self;
        _rootTableView.dataSource = self;
        _rootTableView.sectionIndexColor = NAV_BG_COLOR;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    return _cell;
        ...
        cell.textLabel.textColor = H_SHOPNAME;
...
- (nullable UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    titleLab.textColor = ME_COLOR_CELLTEXTLABEL;
    ...
    if (_HeaderSectionTotal == 3) {
        ...
        switch (section) {
            case 0:
                ...
                break;
            case 1:
                titleLab.text = @"最近访问的城市";
                break;
            case 2:
                titleLab.text = @"热门城市";
                break;
            default:
                titleLab.text = _characterMutableArray[section];
                break;
        }
    }else {
        switch (section) {
    ...
    NSDictionary *titleDic = StringFont_DicK(titleLab.font);
    CGSize titleSize = [titleLab.text  sizeWithAttributes:titleDic];
    
    
    titleLab.frame = FRAMEMAKE_F(20, (40 - titleSize.height) / 2, titleSize.width , titleSize.height);
