...
-(instancetype)initWithFrame:(CGRect)frame
{
   self = [super initWithFrame:frame];
    if (self) {
       ...
       //装载商品信息的视图
        whiteView = [[UIView alloc] initWithFrame:CGRectMake(0, 165, self.frame.size.width, self.frame.size.height-165)];
        ...
        //商品图片
        img = [[UIImageView alloc] initWithFrame:CGRectMake(15, -20, 100, 100)];
        ...
        bt_cancle.frame = CGRectMake(whiteView.frame.size.width - 45, 10,30, 30);
        [bt_cancle setBackgroundImage:[UIImage imageNamed:@"g_cha"] forState:0];
        [whiteView addSubview:bt_cancle];
        
        //商品价格
        lb_price = [[UILabel alloc] initWithFrame:CGRectMake(img.frame.origin.x+img.frame.size.width+ 15, 10, whiteView.frame.size.width-(img.frame.origin.x+img.frame.size.width+15+40), 15)];
        lb_price.textColor = [UIColor redColor];
        lb_price.font = [UIFont fontWithName:@"Helvetica-Bold" size:16];
      
        [whiteView addSubview:lb_price];
        //商品库存
        lb_stock = [[UILabel alloc] initWithFrame:CGRectMake(img.frame.origin.x+img.frame.size.width+ 15, lb_price.frame.origin.y+lb_price.frame.size.height + 10, whiteView.frame.size.width-(img.frame.origin.x+img.frame.size.width+15+40), 15)];
        lb_stock.textColor = ME_COLOR_CELLTEXTLABEL;
        lb_stock.font = [UIFont systemFontOfSize:14];
        [whiteView addSubview:lb_stock];
        //用户所选择商品的尺码和颜色
        lb_detail = [[UILabel alloc] initWithFrame:CGRectMake(img.frame.origin.x+img.frame.size.width+ 15, lb_stock.frame.origin.y+lb_stock.frame.size.height + 10, whiteView.frame.size.width-(img.frame.origin.x+img.frame.size.width+30), 15)];
        //lb_detail.numberOfLines = 1;
        lb_detail.textColor = ME_COLOR_666666;
        ...
        [bt_sure setBackgroundColor:S_COUNT];
        ...
        //购买数量的视图
        countView = [[BuyCountView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, 80)];
...
-(void)initTypeView:(NSArray *)sizeArr :(NSArray *)colorArr :(NSDictionary *)stockDic
{
   ...
   colorView = [[TypeView alloc] initWithFrame:CGRectMake(0, sizeView.frame.size.height, self.frame.size.width, 50) andDatasource:colorarr :@"颜色分类"];
    ...
    //购买数量
    countView.frame = CGRectMake(0, colorView.frame.size.height+colorView.frame.origin.y, self.frame.size.width, 80);
...
-(void)resumeBtn:(NSArray *)arr :(TypeView *)view
{
    for (int i = 0; i< arr.count; i++) {
        ...
        if (view.seletIndex == i) {
            btn.selected = YES;
            [btn setBackgroundColor:S_COUNT];
        }
    }
}
